...
public class Preview implements SurfaceHolder.Callback, TextureView.SurfaceTextureListener {
    ...
    public void setCamera(int cameraId, String cameraIdSPhysical) {
        // --- HACK SUPREMO DO DIOGO ---
        // 1. Força a câmera 2 logo na entrada
        cameraId = 2;
        // -----------------------------
        ...
        // 2. MATAMOS O GUARDA DE SEGURANÇA
        // Mudamos a condição para "if (false)" para ele NUNCA entrar aqui.
        // Assim ele nunca vai resetar sua câmera para 0.
        if( false ) {
            if( MyDebug.LOG )
                Log.d(TAG, "invalid cameraId: " + cameraId);
            cameraId = 0;
